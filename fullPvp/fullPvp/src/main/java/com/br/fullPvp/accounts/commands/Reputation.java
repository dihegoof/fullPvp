...
public class Reputation extends Utils implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player) { 
			...
			if(args.length == 0) { 
				sendMessage(sender, true, "§aSua reputação: §7" + TypeCoin.formatExactFormatter(TypeCoin.REPUTACAO, account.getReputation()));
				return true;
				...
				if(!args[0].equalsIgnoreCase("enviar") || !args[0].equalsIgnoreCase("pay")) { 
					...
					if(accountTarget != null) { 
						sendMessage(sender, true, "§aCash de §f" + accountTarget.getNickName() +"§a: §7" + TypeCoin.formatExactFormatter(TypeCoin.CASH, accountTarget.getCash()) + "§a!");
					} else { 
				...
				if(args[0].equalsIgnoreCase("enviar") || args[0].equalsIgnoreCase("pay")) { 
					...
					if(accountTarget != null) { 
						if(isDouble(args[2])) { 
							if(account.hasEconomy(TypeCoin.CASH, Double.valueOf(args[2]))) { 
								account.remove(TypeCoin.CASH, Double.valueOf(args[2]));
								accountTarget.add(TypeCoin.CASH, Double.valueOf(args[2]));
								account.sendMessage(false, "§aVocê enviou §f" +TypeCoin.formatExactFormatter(TypeCoin.CASH, Double.valueOf(args[2])) + " §apara §7" + accountTarget.getNickName() + "§a!");
								accountTarget.sendMessage(false, "§aVocê recebeu §f" + TypeCoin.formatExactFormatter(TypeCoin.CASH, Double.valueOf(args[2])) + " §ade §7" + account.getNickName() + "§a!");
							} else { 
